...
class RandomForestModel(BaseAnomalyModel):
    ...
    def __init__(
        ...
        logger.info(
            f"RandomForest initialisé: n_estimators={n_estimators}, "
    ...
    def _fit_model(self, X_train, y_train):
        ...
        if y_train is None:
            raise ValueError(
                "Random Forest nécessite des labels d'entraînement (supervisé)"
            )

        logger.info(
            f"Entraînement sur {X_train.shape[0]} échantillons, {X_train.shape[1]} features"
        )
    ...
    def _calculate_feature_importance(self):
        ...
        if self.model and hasattr(self.model, "feature_importances_"):
            ...
            logger.info(
                f"Importance des features calculée pour {len(self.feature_importance_df)} features"
            )
    ...
    def cross_validate(self, X, y, cv_folds=5, scoring="f1"):
        ...
        cv_scores = cross_val_score(
            temp_model, X_scaled, y, cv=skf, scoring=scoring, n_jobs=-1
        )
        ...
        for metric in metrics:
            try:
                scores = cross_val_score(
                    temp_model, X_scaled, y, cv=skf, scoring=metric, n_jobs=-1
                )
        ...
        logger.info(
            f"Validation croisée terminée - {scoring}: {cv_scores.mean():.4f} ± {cv_scores.std():.4f}"
        )
    ...
    def hyperparameter_tuning(self, X, y, param_grid=None, cv_folds=3, scoring="f1"):
        ...
        results = {
            ...
            "score_improvement": grid_search.best_score_
            - np.mean(grid_search.cv_results_["mean_test_score"]),
        }

        logger.info(
            f"Optimisation terminée - Meilleur score: {grid_search.best_score_:.4f}"
        )
    ...
    def analyze_predictions(self, X_test, y_test, save_plots=False):
        ...
        for threshold in thresholds:
            ...
            thresh_metrics = {
                ...
                "precision": precision_score(
                    y_test, thresh_predictions, zero_division=0
                ),
        ...
        analysis = {
            ...
            "false_positives": {
                ...
                "avg_probability": (
                    probabilities[false_positives].mean()
            ...
            "false_negatives": {
                ...
                "avg_probability": (
                    probabilities[false_negatives].mean()
        ...
        logger.info(
            f"Analyse terminée - Précision: {metrics['precision']:.3f}, "
    ...
    def detect_anomalies_with_confidence(self, X, confidence_threshold=0.8):
        ...
        medium_confidence_anomalies = (probabilities >= 0.6) & (
        ...
        results = {
            ...
            "high_confidence": {
                ...
                "avg_probability": (
                    probabilities[high_confidence_anomalies].mean()
            ...
            "medium_confidence": {
                ...
                "avg_probability": (
                    probabilities[medium_confidence_anomalies].mean()
                    if np.any(medium_confidence_anomalies)
                    else 0
                ),
            },
            "low_confidence": {
                ...
                "avg_probability": (
                    probabilities[low_confidence_anomalies].mean()
                    if np.any(low_confidence_anomalies)
                    else 0
                ),
            },
        }
    ...
    def explain_predictions(self, X_sample, feature_names=None, top_n=5):
        ...
        for i, (pred, prob) in enumerate(zip(predictions, probabilities)):
            ...
            if self.feature_importance_df is not None:
                feature_influences = (
                    sample_features * self.feature_importance_df["importance"].values
                )
                    ...
                    top_features.append(
                        {
                            ...
                            "feature_importance": self.feature_importance_df.iloc[idx][
                                "importance"
                            ],
...
def demonstrate_random_forest():
    ...
    print(
        f"✅ F1-Score moyen: {cv_results['primary_mean']:.3f} ± {cv_results['primary_std']:.3f}"
    )
    ...
    confidence_results = model.detect_anomalies_with_confidence(
        X_test, confidence_threshold=0.8
    )
    print(
        f"✅ Haute confiance: {confidence_results['high_confidence']['count']} anomalies"
    )
    print(
        f"✅ Confiance moyenne: {confidence_results['medium_confidence']['count']} anomalies"
    )
